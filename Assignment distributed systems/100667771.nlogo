globals [
  max-sheep           ; Max number of sheep allowed
  locks               ; List of locks held by turtles
  deadlock-detected   ; Flag to indicate a deadlock
  grass-count         ; Number of green patches (grass) in the environment
]

breed [ sheep a-sheep ]  ; sheep is its own plural, so we use "a-sheep" as the singular
breed [ wolves wolf ]    ; wolves breed

turtles-own [energy lock]  ; energy and lock status for each turtle
patches-own [countdown]    ; countdown for grass regrowth
...
to setup
  ...
  ifelse model-version = "sheep-wolves-grass" [
    ask patches [
      set pcolor one-of [green brown]  ; Randomly set patches to green or brown
      ifelse pcolor = green [
        set countdown grass-regrowth-time  ; Initialize countdown for green patches
      ] [
        set countdown random grass-regrowth-time  ; Randomize the countdown for brown patches
      ]
    ]
  ] [
    ask patches [ set pcolor green ]  ; If no grass model, make everything green
  ]
  ...
  ; Initialize the grass count at the start of the simulation
  set grass-count count patches with [pcolor = green]  ; Count the number of green patches
...
to go
  ...
  ; Update grass count in each tick
  set grass-count count patches with [pcolor = green]  ; Update the number of green patches
  ...
  check-deadlock  ; Check for deadlock at each tick

  ; Update the populations graph
  plot-pen-reset
  plot count sheep  ; Plot number of sheep
  plot count wolves  ; Plot number of wolves
...
to move
  acquire-lock "move-lock"  ; acquire a lock before moving
  let grass-patch one-of patches with [pcolor = green]  ; Find a green patch
  if grass-patch != nobody [
    face grass-patch  ; Make the sheep face the grass patch
    fd 1  ; Move forward by one step
  ]
  release-lock "move-lock"  ; release the lock after moving
end

to eat-grass
  if pcolor = green [
    acquire-lock "grass-lock"  ; acquire lock on grass
    set pcolor brown  ; Turn the patch brown after eating (grass is consumed)
    set energy energy + sheep-gain-from-food  ; sheep gain energy by eating
    release-lock "grass-lock"  ; release the lock after eating
  ]
end
...
to eat-sheep
  ...
  if prey != nobody [
    acquire-lock "sheep-lock"  ; acquire lock on sheep
    ask prey [ die ]  ; kill the sheep
    set energy energy + wolf-gain-from-food  ; wolf gains energy from eating
    release-lock "sheep-lock"  ; release the lock after eating
  ]
end
...
to grow-grass
  if pcolor = brown [  ; Only regrow grass on brown patches (eaten by sheep)
    ifelse countdown <= 0 [
      set pcolor green  ; Turn the patch green again
      set countdown grass-regrowth-time  ; Reset the countdown for regrowth
    ] [
      set countdown countdown - 1  ; Decrease countdown as time progresses
    ]
  ]
end

to check-deadlock
  let locked-turtles turtles with [lock != "none"]  ; find all locked turtles
  if count locked-turtles = count turtles [  ; if all turtles are locked
    set deadlock-detected true
    resolve-deadlock
  ]
end

to resolve-deadlock
  let turtle-to-release one-of turtles with [lock != "none"]  ; choose a turtle randomly
  ask turtle-to-release [
    release-lock lock  ; force this turtle to release its lock
    print (word "Deadlock detected! Forcing " who " to release lock.")
  ]
  set deadlock-detected false  ; reset the deadlock flag
end

to acquire-lock [resource]
  ifelse lock = "none" [
    set lock resource  ; acquire the lock
  ]
  [
    ; If the turtle already has a lock, it waits
    ; For simplicity, we don't add complex waiting logic here, but this can be implemented as needed
  ]
end

to release-lock [resource]
  set lock "none"  ; release the lock
end
...
;; BehaviorSpace Experiment: Adjust parameters for running experiments
to run-experiment
  ca  ;; clear all settings
  setup
  repeat 100 [  ;; run for 100 ticks as an example
    go
  ]
...
"" ""
PENS
"sheep" 1.0 0 -612749 true "go" "plot count sheep"
"wolves" 1.0 0 -16449023 true "go" "plot count wolves"
"count-grass" 1.0 0 -7500403 true "go" "plot grass-count"
...
<experiments>
  <experiment name="New BehaviorSpace Features" repetitions="3" runMetricsEveryStep="false">
    ...
    <metric>count grass-count</metric>
